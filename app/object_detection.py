...
class ObjectDetection(object):
    """Class for Custom Vision's object detection model
    """
    ...
    def __init__(self, labels, prob_threshold=0.10, max_detections = 20):
        ...
        Args:
            labels ([str]): list of labels for the model.
    ...
    def postprocess(self, prediction_outputs):
        ...
        sc=0
        cob=0
        if(len(selected_boxes)==0):con1=("No signs of depression detected")
        if(len(selected_boxes)==1):con2=("Risk of depression detected")
        if(len(selected_boxes)==2):con3=("Mild signs of depression detected")
        if(len(selected_boxes)==3):con4=("Moderate signs of depression detected")
        if(len(selected_boxes)==4):con5=("Severe signs of depression detected\n")
        List = []
        for i in range(len(selected_boxes)):
            if(selected_probs[i]>0.5):List.append(self.labels[selected_classes[i]])#Probability factor
        """print(List)"""
        for i in range(len(List)):
            dt= List.count("Dead tree")
            s= List.count("Shadow")
            sc= List.count("Shaded crown")
            cob= List.count("Cut off branch")
        if(dt>0):conc1="Depressed with feelings of guilty and suicidal thoughts"
        if(s>0):conc2="Neurosis"
        if(sc>0):conc3="Shaded crown"
        if(cob>0):conc4="Cut off branch"
        
     
        x=([{'tagName': self.labels[selected_classes[i]],
       ...
       # return  '{} {} {} {} {} {} {} {} {} {}'.format(con1,con2,con3,con4,con5,conc1,conc2,conc3,conc4,x) 
        return {
            'con1': con1,
            'con2': con2,
            'con3': con3,
